...
tf.compat.v1.flags.DEFINE_string('train_dir', '/tmp/bichen/logs/squeezeseg/train',
                            """Directory where to write event logs """
                            """and checkpoint.""")
tf.compat.v1.flags.DEFINE_integer('max_steps', 1000000,
                            """Maximum number of batches to run.""")
tf.compat.v1.flags.DEFINE_string('net', 'squeezeSeg',
                           """Neural net architecture. """)
tf.compat.v1.flags.DEFINE_string('pretrained_model_path', '',
                           """Path to the pretrained model.""")
tf.compat.v1.flags.DEFINE_integer('summary_step', 50,
...
def train():
  ...
  with tf.Graph().as_default():
    ...
    saver = tf.compat.v1.train.Saver(tf.compat.v1.global_variables(), max_to_keep=10000)
    ...
    run_options = tf.compat.v1.RunOptions(timeout_in_ms=60000)
...
def main(argv=None):  # pylint: disable=unused-argument
  if tf.io.gfile.exists(FLAGS.train_dir):
    tf.compat.v1.gfile.DeleteRecursively(FLAGS.train_dir)
