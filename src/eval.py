...
tf.compat.v1.app.flags.DEFINE_string('data_path', '', """Root directory of data""")
tf.compat.v1.app.flags.DEFINE_string('image_set', 'val',
                           """Can be train, trainval, val, or test""")
tf.compat.v1.app.flags.DEFINE_string('eval_dir', '/tmp/bichen/logs/squeezeSeg/eval',
                            """Directory where to write event logs """)
tf.compat.v1.app.flags.DEFINE_string('checkpoint_path', '/tmp/bichen/logs/squeezeSeg/train',
                            """Path to the training checkpoint.""")
tf.compat.v1.app.flags.DEFINE_integer('eval_interval_secs', 60 * 1,
...
def evaluate():
  ...
  with tf.Graph().as_default() as g:
    ...
    for sm in eval_summary_names:
      ...
      eval_summary_ops.append(tf.compat.v1.summary.scalar(sm, ph))

    saver = tf.compat.v1.train.Saver(model.model_params)

    summary_writer = tf.compat.v1.summary.FileWriter(FLAGS.eval_dir, g)
