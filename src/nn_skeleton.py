...
class ModelSkeleton:
  ...
  def __init__(self, mc):
    ...
    self.ph_keep_prob = tf.compat.v1.placeholder(tf.float32, name='keep_prob')
    # projected lidar points on a 2D spherical surface
    self.ph_lidar_input = tf.compat.v1.placeholder(
  ...
  def _add_loss_graph(self):
    ...
    # Define Focal Loss
    with tf.compat.v1.variable_scope('cls_loss') as scope:
      ...
      tf.compat.v1.add_to_collection('losses', self.cls_loss)

    # add above losses as well as weight decay losses to form the total loss
    self.loss = tf.add_n(tf.compat.v1.get_collection('losses'), name='total_loss')
    # add loss summaries
    # _add_loss_summaries(self.loss)
    tf.compat.v1.summary.scalar(self.cls_loss.op.name, self.cls_loss)
  ...
  def _add_viz_graph(self):
    ...
    mc = self.mc
    self.label_to_show = tf.compat.v1.placeholder(
        tf.float32, [None, mc.ZENITH_LEVEL, mc.AZIMUTH_LEVEL, 3],
        name='label_to_show'
    )
    self.depth_image_to_show = tf.compat.v1.placeholder(
        tf.float32, [None, mc.ZENITH_LEVEL, mc.AZIMUTH_LEVEL, 1],
        name='depth_image_to_show'
    )
    self.pred_image_to_show = tf.compat.v1.placeholder(
        tf.float32, [None, mc.ZENITH_LEVEL, mc.AZIMUTH_LEVEL, 3],
        name='pred_image_to_show'
    )
    self.show_label = tf.compat.v1.summary.image('label_to_show',
        self.label_to_show, collections='image_summary',
        max_outputs=mc.BATCH_SIZE)
    self.show_depth_img = tf.compat.v1.summary.image('depth_image_to_show',
        self.depth_image_to_show, collections='image_summary',
        max_outputs=mc.BATCH_SIZE)
    self.show_pred = tf.compat.v1.summary.image('pred_image_to_show',
  ...
  def _add_summary_ops(self):
    ...
    for cls in mc.CLASSES:
      ph = tf.compat.v1.placeholder(tf.float32, name=cls+'_iou')
      iou_summary_placeholders.append(ph)
      iou_summary_ops.append(
          tf.compat.v1.summary.scalar('Eval/'+cls+'_iou', ph, collections='eval_summary')
      )
  ...
  def _conv_bn_layer(
    ...
    with tf.compat.v1.variable_scope(conv_param_name) as scope:
      ...
      beta  = _variable_on_device('beta', [filters], beta_val,
                                  trainable=(not freeze))
      moving_mean = _variable_on_device('moving_mean', [1, 1, 1, filters], tf.compat.v1.zeros_initializer,
                                        trainable=False)
      moving_var = _variable_on_device('moving_var', [1, 1, 1, filters], tf.compat.v1.ones_initializer,
                                        trainable=False)
      self.model_params += [gamma, beta, moving_mean, moving_var]
      conv = tf.nn.conv2d(
          inputs, filters=kernel, strides=[1, 1, stride, 1], padding=padding,
          name='convolution')
      if conv_with_bias:
      ...
      if mc.TRAINING: 
        ...
        updated_mean = tf.compat.v1.assign(moving_mean, moving_mean * decay + mean * (1 - decay))
        updated_var = tf.compat.v1.assign(moving_var, moving_var * decay + var * (1 - decay))  
  ...
  def _conv_layer(
    ...
    with tf.compat.v1.variable_scope(layer_name) as scope:
      ...
      if use_pretrained_param:
        ...
        bias_init = tf.compat.v1.constant_initializer(bias_init_val)
      else:
        kernel_init = tf.compat.v1.truncated_normal_initializer(
            stddev=stddev, dtype=tf.float32)
        bias_init = tf.compat.v1.constant_initializer(bias_init_val)
      ...
      if init:
        ...
        bias_init = tf.compat.v1.constant_initializer(-np.log((1 - bias_init_val)/bias_init_val))
      kernel = _variable_with_weight_decay(
          'kernels', shape=[size, size, int(channels), filters],
          wd=mc.WEIGHT_DECAY, initializer=kernel_init, trainable=(not freeze))
  ...
  def _pooling_layer(
    ...
    with tf.compat.v1.variable_scope(layer_name) as scope:
      out =  tf.nn.max_pool2d(input=inputs, 
                            ksize=[1, size, size, 1], 
    ...
    with tf.compat.v1.variable_scope(layer_name) as scope:
      ...
      for it in range(num_iterations):
        ...
        ang_output = tf.nn.conv2d(
            ang_output, filters=angular_compat_kernel, strides=[1, 1, 1, 1],
        ...
        bi_output = tf.nn.conv2d(
            bi_output, filters=bi_compat_kernel, strides=[1, 1, 1, 1], padding='SAME',
  ...
  def _locally_connected_layer(
    ...
    with tf.compat.v1.variable_scope(layer_name) as scope:
      # message passing
      ang_output = tf.nn.conv2d(
          inputs, filters=angular_filters, strides=[1, 1, 1, 1], padding=padding,
          name='angular_filtered_term'
      )

      bi_ang_output = tf.nn.conv2d(
          inputs, filters=bi_angular_filters, strides=[1, 1, 1, 1], padding=padding,
          name='bi_angular_filtered_term'
      )

      condensed_input = tf.reshape(
          tf.nn.conv2d(
              inputs*self.lidar_mask, filters=condensing_kernel, strides=[1, 1, 1, 1], padding=padding,
  ...
  def _bilateral_filter_layer(
    ...
    with tf.compat.v1.variable_scope(layer_name) as scope:
      ...
      condensed_input = tf.nn.conv2d(
          inputs, filters=condensing_kernel, strides=[1, 1, stride, 1], padding=padding,
  ...
  def _activation_summary(self, x, layer_name):
    ...
    with tf.compat.v1.variable_scope('activation_summary') as scope:
      tf.compat.v1.summary.histogram(layer_name, x)
      tf.compat.v1.summary.scalar(layer_name+'/sparsity', tf.nn.zero_fraction(x))
      tf.compat.v1.summary.scalar(layer_name+'/average', tf.reduce_mean(x))
      tf.compat.v1.summary.scalar(layer_name+'/max', tf.reduce_max(x))
      tf.compat.v1.summary.scalar(layer_name+'/min', tf.reduce_min(x))
